...
(function run() {
  ...
  // Проверка наличия самолетов в аэропорте
  if (militaryAirport.getPlanes().length === 0) {
    console.log('Military airport has no planes.');
  } else {
    console.log(
      `Military airport sorted by max distance: ${Airport.print(
        militaryAirport.sortByMaxDistance().getPlanes()
      )}`
    );
  }

  if (passengerAirport.getPlanes().length === 0) {
    console.log('Passenger airport has no planes.');
  } else {
    console.log(
      `Passenger airport sorted by max speed: ${Airport.print(
        passengerAirport.sortByMaxSpeed().getPlanes()
      )}`
    );
    console.log(
      `Plane with max passenger capacity: ${Airport.print([
        passengerAirport.getPassengerPlaneWithMaxPassengersCapacity(),
      ])}` // Оборачиваем результат в массив
    );
  }
})();
